...
def _build_activity(filename: str, points: list[TrackPoint], source_distance_m: float | None) -> Activity:
...
def parse(filename: str, data: bytes) -> Activity:
    ...
    if ext == 'fit':
        ...
        return parse_fit(filename, data)
    if ext == 'gpx':
        from bot.parsers.gpx import parse_gpx

        return parse_gpx(filename, data)
    if ext == 'tcx':
        from bot.parsers.tcx import parse_tcx

        return parse_tcx(filename, data)
